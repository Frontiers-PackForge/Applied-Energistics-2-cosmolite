...
分子装配室会接收输入物品，并执行相邻 <ItemLink id="pattern_provider" /> 指定的操作；也可执行直接插入其中的 <ItemLink id="crafting_pattern" />、<ItemLink id="smithing_table_pattern" /> 与 <ItemLink id="stonecutting_pattern" />，然后将产物输出到相邻容器。

下述装配室装有“1x 橡木原木 = 4x 橡木木板”样板。向上方漏斗放入橡木原木后，分子装配室会开始合成，并将木板弹出到下方漏斗。
...
OptiFine 会破坏“弹出至相邻容器”功能，导致大多数需要装配室的合成设施无法工作。