...
虽然相邻的 AE2 机器也能形成 ME 网络，但要大范围扩展网络，线缆仍是主要方式。

不同颜色的线缆可避免相邻线缆自动连接，从而让[频道](../ae2-mechanics/channels.md)分配更高效。线缆颜色也会影响其上终端的外观，不会总是紫色。福鲁伊克斯色线缆可与所有颜色线缆相连。

需要注意，**频道和线缆颜色没有关系**。

## 重要备注

**如果你新入门 AE2，还不熟悉频道的话，可以在各处尽量使用智能线缆和致密线缆。它们会显示频道在网络中的路径，方便理解频道的行为。**

## 另一则备注

**频道不是物品/流体/能量/其他类型的管道。**频道没有内部存储空间，样板供应器和机器不会向频道“输入”物品，频道唯一做的事情便是将 AE2 [设备](../ae2-mechanics/devices.md)连成一个网络。
...
<ItemLink id="fluix_glass_cable" />是最简单的线缆，能传输能量，最多可传输 8 个[频道](../ae2-mechanics/channels.md)。它共有 17 种颜色，默认为福鲁伊克斯色，且可用 16 种染料染成相应颜色。

可在合成格中用 8 根线缆围绕染料合成染色线缆（线缆颜色可混用，但必须是同一线缆类型，如玻璃线缆或智能线缆）。也可用任意 Forge 兼容颜料刷为世界中的线缆染色。
...
高容量线缆，能传输 32 个频道，而非普通线缆的 8 个。但是致密线缆不支持总线，必须先将致密线缆降为小型线缆（例如<ItemLink id="fluix_glass_cable" />和<ItemLink id="fluix_smart_cable" />），才能放上总线和面板。
