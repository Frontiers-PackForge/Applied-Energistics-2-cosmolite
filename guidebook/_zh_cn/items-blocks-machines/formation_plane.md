...
注意，这些设备在[管道子网](../example-setups/pipe-subnet.md)中类似「导入总线 → 存储总线」和「接口 → 存储总线」。
...
在[管道子网](../example-setups/pipe-subnet.md)等设施中，此[设备](../ae2-mechanics/devices.md)的行为方式类似存储总线；如果需求是放置方块或投出物品而非传输，也能替代存储总线。
...
默认情况下，成型面板不会放置或投出任何东西。放入其过滤槽的物品会加入白名单，也即只会放置其中指明的内容。
...
单击 GUI 右上角扳手图标以设置优先级。进入网络的物品会优先送至优先级最高的存储位置。
