...
接口还有一个实用特性：大多数流体储罐只能存 1 种流体，而接口最多可存 9 种；物品同理。它本质上是带有额外功能的箱子/多流体储罐，并且可通过断开网络连接来禁用这些额外功能。因此在“少量多种内容”的存储场景里非常有用。

## 接口内部的工作原理

正如前文所提，接口本质上是箱子和储罐，再附上一组 <ItemLink id="import_bus" />、<ItemLink id="export_bus" /> 与 <ItemLink id="level_emitter" /> 的逻辑。
...
接口与其他 AE2 [设备](../ae2-mechanics/devices.md)之间有若干特殊交互：
...
样板供应器和接口有一特殊交互效果——[子网](../ae2-mechanics/subnetworks.md)：如果接口未经修改（请求槽内无内容），则供应器会跳过接口，直接输出到该子网的[网络存储](../ae2-mechanics/import-export-storage.md)，而非输出到接口的存储槽；更重要的是，只要对应的网络存储没有足够空间，下一批物品就不会继续输出。
...
接口有 2 种变体：普通、面板/[子部件](../ae2-mechanics/cable-subparts.md)。这会影响它在各面“输出材料、接收物品、提供网络连接”的能力。

*   普通型接口会向各面输出材料，会从各面接收物品，且和大多数 AE2 机器一样向各面提供网络连接，类似线缆。

*   面板型接口是[线缆子部件](../ae2-mechanics/cable-subparts.md)，因此可在同一线缆上放置多个，便于设计紧凑设施。它们可从自身存储空间输出、向自身存储空间接收物品，并允许其他装置访问其存储空间，但不提供网络连接。

接口的普通和面板形态可在合成方格中转换。

## 设置

接口上排槽位设定需要存储于自身的物品。可直接放入或从 JEI/REI 中拖拽放入，有物品的槽位上方会出现扳手图标，可用其设置数量。
...
可点击 GUI 右上角扳手以设置优先级。高优先级的接口会先于低优先级接口获取物品。
