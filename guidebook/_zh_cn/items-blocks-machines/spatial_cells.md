...
空间元件可用于[存储物理空间中的某一区域](../ae2-mechanics/spatial-io.md)，并用于 <ItemLink id="spatial_io_port" />。

与[存储元件](../items-blocks-machines/storage_cells.md)不同，空间元件不可重新格式化。
...
空间组件是空间元件的核心。每一级组件容量的边长是前一级的 8 倍。
