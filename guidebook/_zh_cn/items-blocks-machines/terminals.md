...
<ItemLink id="pattern_provider" />、<ItemLink id="import_bus" />、<ItemLink id="storage_bus" /> 等是 AE2 网络与世界交互的基础方式，而终端则是 AE2 网络与*你*交互的基础方式。终端有多种功能各异的变体。
...
终端通常是最先放置的[子部件](../ae2-mechanics/cable-subparts.md)，因此出现放置角度不对或位置不理想的情况很常见。下方给出“推荐/不推荐”示例：
...
搜索框支持正则表达式，因此可输入 `gtceu:.*ore` 来搜索所有 GregTech 矿石。正则表达式的详细用法这里不展开。
...
基础终端可让你访问[网络存储](../ae2-mechanics/import-export-storage.md)内容，也可向[自动合成](../ae2-mechanics/autocrafting.md)系统发送请求。
...
*   左击拿出一组，右击拿出半组
*   如果某物品或流体支持[自动合成](../ae2-mechanics/autocrafting.md)，按“选取方块”键（通常是鼠标中键）会唤起数量设置界面。可输入 `3*64/2` 之类的公式，也可输入 `=32` 让系统补齐到 32 个
...
*   合成模式：
    *   左击或从 JEI/REI 拖拽以将材料加至配方；右击以移除材料
    *   启用「允许替代」后，可支持“任意木板合成木棍”这类配方；应当仅在绝对必要时才启用
    *   流体替代允许使用存储中的流体替代桶装流体
    *   也可直接从 JEI/REI 配方界面中编码样板

*   处理模式：
    * 左击、右击或从 JEI/REI 拖拽以设定配方的输入和输出
