...
将[接口的特殊行为](../items-blocks-machines/interface.md#special-interactions)加以运用，就能让[子网](../ae2-mechanics/subnetworks.md)不可访问主网络存储，而其存储内容又在主网络中可见，且仅占 1 条[频道](../ae2-mechanics/channels.md)。
...
<BoxAnnotation color="#dddddd" min="3 1 0" max="4 2 0.3">
        （3）合成终端：此终端可查看子网中驱动器的内容物，但无法查看主网络存储。
  </BoxAnnotation>
...
* 第一个<ItemLink id="interface" />（1）只会接受农场产物并送入子网。
...
* 子网上的<ItemLink id="interface" />会向主网上的<ItemLink id="storage_bus" />展示<ItemLink id="drive" />的内容物。也即存储总线可直接向该驱动器中的元件输入输出。
* 存储总线的[优先级](../ae2-mechanics/import-export-storage.md#storage-priority)设为较高，使物品优先回到子网而非主网络存储。
* 重要的是，假如子网的元件被填满了，多出的物品不会溢出至主网络。如果农场会因堵塞而失效，可装入<ItemLink id="void_card" />以删除多余物品。
