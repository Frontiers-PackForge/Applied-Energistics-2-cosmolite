...
public class InterfaceAccessTerminalPart extends AbstractDisplayPart implements IConfigurableObject {
    ...
    public InterfaceAccessTerminalPart(IPartItem<?> partItem) {
        super(partItem, true);
        this.configManager.registerSetting(Settings.TERMINAL_STYLE, TerminalStyle.MEDIUM);
    }

    @Override
    public boolean onPartActivate(Player player, InteractionHand hand, Vec3 pos) {
        if (!super.onPartActivate(player, hand, pos) && !isClientSide()) {
            MenuOpener.open(InterfaceAccessTermMenu.TYPE, player, MenuLocators.forPart(this));
        }
        return true;
    }
